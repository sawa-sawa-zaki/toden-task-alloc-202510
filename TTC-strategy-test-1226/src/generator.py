...
def ranks_to_matrix(ranks, base=10.0):
    ...
    修正点:
    個人の「受容ランク数」に関わらず、第一希望(Rank 0)のスコアが全エージェントで
    統一されるように、システム全体の総スロット数(6)を基準に計算する。
    ...
    # スコア計算
    # 配列の要素数(6)を基準にする (TotalSlots - 1 - rank)
    total_slots = len(ranks) 
    max_exponent = total_slots - 1
    
    # 一旦すべて計算 (辞退箇所はマイナス乗になるが後で0にするのでOK)
    scores = base ** (max_exponent - arr.astype(float))
