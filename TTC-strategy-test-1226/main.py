...
OUTSIDE_PENALTY = 10000  # 真実で outside(=0) のスロットに割当が入ったら減点
...
def utility_with_outside_penalty(avg_alloc, true_val_matrix_a, outside_penalty):
    """
    avg_alloc: shape (A, M, T)
    true_val_matrix_a: shape (M, T)

    旧：sum(alloc[0] * true_val_matrix_a)
    新：上に加えて、true_val_matrix_a==0 のスロットに割当が入っていれば減点
    """
...
def main():
    ...
    print(f"Outside penalty: -{OUTSIDE_PENALTY} per allocated unit to outside(true==0)")
    ...
    with open(config_log_path, "w") as f:
        ...
        f.write(f"OUTSIDE_PENALTY: {OUTSIDE_PENALTY}\n")
    ...
    # --- 4. Outcome Matrix（期待配分を先に全部計算してキャッシュ） ---
    unique_matrices_A = []
    mat_to_unique_idx = {}
    ...
    # ここが「重い」フェーズ：しかし 14分版はここだけ回していた
    ...
    for p_idx, (idx_true_a, idx_true_b) in tqdm(
        ...
        u_truth = utility_with_outside_penalty(
            alloc_truth, val_matrix_a, OUTSIDE_PENALTY
        )
        ...
        for idx_lie in range(n_strategy):
            ...
            u_lie = utility_with_outside_penalty(
                alloc_lie, val_matrix_a, OUTSIDE_PENALTY
            )
        ...
        if is_violation:
            ...
            if violation_type == "Type A":
                count_type_a += 1
            elif violation_type == "Type B":
                count_type_b += 1

        true_str = ranks_to_string(truth_types[idx_true_a]['ranks'], T=cfg.T, M=cfg.M)
        lie_str = ranks_to_string(strategy_types[best_lie_idx]['ranks'], T=cfg.T, M=cfg.M)
